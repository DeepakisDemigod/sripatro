...
const Calendar = () => {
  ...
  const today = new Date(); // 06:33 PM IST, June 20, 2025
  ...
  const renderSelectedDateEvents = () => {
    // Always render events panel with selectedDate or today's date if none selected
    const displayDate = selectedDate || days.find((d) => d && d.day === today.getDate() && viewYear === today.getFullYear() && viewMonth === today.getMonth());
          ...
          {viewYear >= 2000 && viewYear <= 2090 && holiday && (
            <>
              ...
              <p className="text-gray-700">
                ...
                <span className={`badge ${isHoliday ? "bg-red-700 text-white" : ""}`}>
                  {isHoliday ? (
        ...
        <div className="lg:flex-1">
          <div className="mb-6">
            <div className="flex justify-between items-center gap-4">
              ...
              >
                {Array.from({ length: 2090 - 2000 + 1 }, (_, i) => 2000 + i).map(
                  (year) => (
          ...
          <div className="w-[330px] grid grid-cols-7 shadow-lg">
            {dayKeys.map((day) => (
              <div
                key={day}
            ...
            {days.length > 0 ? (
		      ...
		      <div className="text-xl">{dayObj?.day ||      ""}</div>
                  <div className="text-xs text-base-500">{dayObj?.nepDate}</div>
		      </div>
                      ...
                      {dayObj ? (
                        <div>
                          <div className="flex items-end justify-center">
                            <div className="text-xl">{dayObj.day}</div>
                            <div className="text-xs text-base-500 ml-1">{dayObj.nepDate}</div>
                          </div>
                          <div className="text-sm opacity-75">
                            {dayObj.holiday?.name || ""}
                          </div>
