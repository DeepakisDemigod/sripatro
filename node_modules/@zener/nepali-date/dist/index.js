...
// src/nepali-date/index.ts
var nepali_date_exports = {};
__export(nepali_date_exports, {
  AD_MONTH: () => AD_MONTH,
...
module.exports = __toCommonJS(nepali_date_exports);
...
var engToNepaliNumber = (number) => {
  return number.toString().split("").map((n) => NEPALI_DIGITS[n]).join("");
};
...
// src/nepali-date/nepali-date.ts
var NepaliDate = class _NepaliDate {
  #year = 2e3;
  #month = 0;
  #date = 1;
  #adDate = /* @__PURE__ */ new Date();
  constructor(yearOrValue, month, date) {
    if (typeof yearOrValue === "number") {
      ...
      this.#date = date;
      this.#setADDate();
    } else if (typeof yearOrValue === "string" || yearOrValue instanceof _NepaliDate || yearOrValue instanceof Date) {
      ...
      } else if (yearOrValue instanceof _NepaliDate) {
  ...
  getDay(lang = "en") {
    return lang === "en" ? this.#adDate.getDay() : engToNepaliNumber(this.#adDate.getDay());
  }
  getDayName(lang = "en") {
  ...
  #getDayByIndex(index, lang = "en", full = false) {
    if (index < 0 || index > 6) {
  ...
  subtract(value, type) {
    ...
    return new _NepaliDate(m.year, m.month, m.date);
  }
  add(value, type) {
    const v = Math.abs(value);
    const m = this.#manipulate(v, type);
