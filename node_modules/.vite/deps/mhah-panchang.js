...
// node_modules/mhah-panchang/dist/mhah-panchang.esm.js
var corr = function corr2(mlcor, mscor, fcor, dcor, lcor) {
  this.mlcor = mlcor;
  this.mscor = mscor;
  this.fcor = fcor;
  this.dcor = dcor;
...
var corr22 = function corr23(l, ml, ms, f, d) {
  this.l = l;
  this.ml = ml;
  this.ms = ms;
  this.f = f;
...
var MhahMoonOutragConst = function MhahMoonOutragConst2() {
  ...
  this.corrMoon2 = [new corr22(0.127, 0, 0, 0, 6), new corr22(-0.151, 0, 2, 0, -4), new corr22(-0.085, 0, 0, 2, 4), new corr22(0.15, 0, 1, 0, 3), new corr22(-0.091, 2, 1, 0, -6), new corr22(-0.103, 0, 3, 0, 0), new corr22(-0.301, 1, 0, 2, -4), new corr22(0.202, 1, 0, -2, -4), new corr22(0.137, 1, 1, 0, -1), new corr22(0.233, 1, 1, 0, -3), new corr22(-0.122, 1, -1, 0, 1), new corr22(-0.276, 1, -1, 0, -3), new corr22(0.255, 0, 0, 2, 1), new corr22(0.254, 0, 0, 2, -3), new corr22(-0.1, 3, 1, 0, -4), new corr22(-0.183, 3, -1, 0, -2), new corr22(-0.297, 2, 2, 0, -2), new corr22(-0.161, 2, 2, 0, -4), new corr22(0.197, 2, -2, 0, 0), new corr22(0.254, 2, -2, 0, -2), new corr22(-0.25, 1, 3, 0, -2), new corr22(-0.123, 2, 0, 2, 2), new corr22(0.173, 2, 0, -2, -4), new corr22(0.263, 1, 1, 2, 0), new corr22(0.13, 3, 0, 0, -1), new corr22(0.113, 5, 0, 0, 0), new corr22(0.092, 3, 0, 2, -2)];
};
var MhahPanchangImpl = function() {
  function MhahPanchangImpl2(mhahConstant) {
    this.mhahConstant = mhahConstant;
    this.mhahMoonOutragConst = new MhahMoonOutragConst();
    this.d2r = Math.PI / 180;
    this.r2d = 180 / Math.PI;
    this.range = [1, 31, 0, 0, -3e3, 4e3, 0, 23, 0, 59, -12, 12, 0, 59];
    this.tipnaks = [2, 5, 6, 0, 1, 4, 3, 2, 4, 5, 5, 0, 2, 1, 3, 6, 1, 4, 4, 5, 0, 3, 3, 3, 5, 0, 1];
    this.ayanamsa = 0;
  }
  var _proto = MhahPanchangImpl2.prototype;
  ...
  _proto.moon = function moon(jd) {
    ...
    var t2 = t * t;
    var t3 = t * t * t;
    var l = 270.4337361 + 13.1763965445281 * tdays - 5.86 * t2 / 3600 + 68e-4 * t3 / 3600;
    var d = 350.7374861110581 + 445267.1142166667 * t - t2 * 0.001436111132303874 + 18888889e-13 * t3;
    var pe = 334.329556 + 1464852252e-2 * t / 3600 - 37.17 * t2 / 3600 - 0.045 * t3 / 3600;
    var ms = 358.4758333333334 + 35999.04974999958 * t - t2 * 1500000059604645e-19 - t3 * 33333333623078e-19;
    var ml = this.fix360(l - pe);
    var om = 259.183275 - 696291123e-2 * t / 3600 + 7.48 * t2 / 3600 + 8e-3 * t3 / 3600;
    var f = this.fix360(l - om);
    var r2rad = 360 * this.d2r;
    var tb = tdays * 1e-12;
    var t2c = tdays * tdays * 1e-16;
    ...
    var dgc = (-4.318 * c2 - 0.698 * c4) / 3600 / 360;
    dgc = 1.000002708 + 139.978 * dgc;
    ml = this.d2r * (ml + (dlm - dpm) / 3600);
    ms = this.d2r * (ms + dls / 3600);
    f = this.d2r * (f + (dlm - dkm) / 3600);
    d = this.d2r * (d + (dlm - dls) / 3600);
    var lk = 0;
    var lk1 = 0;
    var i1corr = 1 - 6832e-11 * tdays;
    var i2corr = dgc * dgc;
    for (var i = 0; i < 93; i++) {
    ...
    dlid += 0.348 * Math.sin(r2rad * (0.68266 - 6873156e-10 * tdays));
    ...
    dlid += 0.327 * Math.sin(r2rad * (0.97639 + 173491e-9 * tdays));
    l = l + this.nutation(jd) + (dlm + lk + lk1 + dlid) / 3600;
    this.LmoonYoga = l;
    l = this.fix360(l);
    var vl = 13.176397;
    ...
    vl = vl + 8649e-6 * Math.cos(ms - ml);
    vl = vl - 8642e-6 * Math.cos(2 * f + ml);
    vl = vl - 7471e-6 * Math.cos(ms + ml);
    vl = vl - 7387e-6 * Math.cos(d);
    vl = vl + 6864e-6 * Math.cos(3 * ml);
    vl = vl + 665e-5 * Math.cos(4 * d - ml);
    vl = vl + 3523e-6 * Math.cos(2 * d + 2 * ml);
    vl = vl + 3377e-6 * Math.cos(4 * d - 2 * ml);
    vl = vl + 3287e-6 * Math.cos(4 * d);
    vl = vl - 3193e-6 * Math.cos(ms);
    vl = vl - 3003e-6 * Math.cos(2 * d + ms);
    vl = vl + 2577e-6 * Math.cos(ml - ms + 2 * d);
    vl = vl - 2567e-6 * Math.cos(2 * f - ml);
    vl = vl - 1794e-6 * Math.cos(2 * d - 2 * ml);
    vl = vl - 1716e-6 * Math.cos(ml - 2 * f - 2 * d);
    vl = vl - 1698e-6 * Math.cos(2 * d + ms - ml);
    vl = vl - 1415e-6 * Math.cos(2 * d + 2 * f);
    vl = vl + 1183e-6 * Math.cos(2 * ml - ms);
    vl = vl + 115e-5 * Math.cos(d + ms);
    vl = vl - 1035e-6 * Math.cos(d + ml);
    vl = vl - 1019e-6 * Math.cos(2 * f + 2 * ml);
    vl = vl - 1006e-6 * Math.cos(ms + 2 * ml);
    this.skor = vl;
    return l;
  };
  _proto.sun = function sun(jd) {
    ...
    var g = ms + (0.266 * Math.sin((31.8 + 119 * t) * this.d2r) + 6.4 * Math.sin((231.19 + 20.2 * t) * this.d2r) + (1.882 - 0.016 * t) * Math.sin((57.24 + 150.27 * t) * this.d2r)) / 3600;
    var ex = 0.01675104 - 418e-7 * t - 126e-9 * t2;
    var u = this.kepler(g, ex, 3e-7);
    var truanom;
    var b = Math.sqrt((1 + ex) / (1 - ex));
    if (Math.abs(Math.PI - u) < 1e-10) truanom = u;
    else truanom = 2 * Math.atan(b * Math.tan(u / 2));
    ...
    var u4 = (350.74 + 445267.1142 * t - 144e-5 * t2) * this.d2r;
    var u6 = (353.4 + 65928.7155 * t) * this.d2r;
    var u5 = (315.6 + 893.3 * t) * this.d2r;
    var dl = 134e-5 * Math.cos(u1);
    dl += 154e-5 * Math.cos(u2);
    dl += 2e-3 * Math.cos(u3);
    dl += 179e-5 * Math.sin(u4);
    dl += 0.202 * Math.sin(u5) / 3600;
    var dr = 543e-8 * Math.sin(u1);
    dr += 1575e-8 * Math.sin(u2);
    dr += 1627e-8 * Math.sin(u3);
    dr += 3076e-8 * Math.cos(u4);
    dr += 926999999e-14 * Math.sin(u6);
  ...
  _proto.novolun = function novolun(jd, knv) {
    ...
    var t2 = t * t;
    var t3 = t * t * t;
    var jdnv = 241502075933e-5 + 29.53058868 * knv + 1178e-7 * t2 - 155e-9 * t3;
    jdnv += 33e-5 * Math.sin((166.56 + 132.87 * t - 9173e-6 * t2) * this.d2r);
    var m = 359.2242 + 29.10535608 * knv - 333e-7 * t2 - 347e-8 * t3;
    var ml = 306.0253 + 385.81691806 * knv + 0.0107306 * t2 + 1236e-8 * t3;
    var f = 21.2964 + 390.67050646 * knv - 16528e-7 * t2 - 239e-8 * t3;
    m *= this.d2r;
    ml *= this.d2r;
    f *= this.d2r;
    var djd = (0.1734 - 393e-6 * t) * Math.sin(m);
    djd += 21e-4 * Math.sin(2 * m);
    djd -= 0.4068 * Math.sin(ml);
    djd += 0.0161 * Math.sin(2 * ml);
    djd -= 4e-4 * Math.sin(3 * ml);
    djd += 0.0104 * Math.sin(2 * f);
    djd -= 51e-4 * Math.sin(m + ml);
    djd -= 74e-4 * Math.sin(m - ml);
    djd += 4e-4 * Math.sin(2 * f + m);
    djd -= 4e-4 * Math.sin(2 * f - m);
    djd -= 6e-4 * Math.sin(2 * f + ml);
    djd += 1e-3 * Math.sin(2 * f - ml);
    djd += 5e-4 * Math.sin(m + 2 * ml);
    jdnv += djd;
    return jdnv;
  };
  ...
  _proto.nutation = function nutation(jd) {
    var t = (jd - 2415020) / 36525;
    var t2 = t * t;
    var ls = 279.6967 + 36000.7689 * t + 303e-6 * t2;
    var l = 270.4341639 + 481267.8831417 * t - 0.0011333333 * t2;
    var ms = 358.4758333333334 + 35999.04974999958 * t - t2 * 1500000059604645e-19;
    var ml = 296.1046083333757 + 477198.8491083336 * t + 0.0091916667090522 * t2;
    var d = 350.7374861110581 + 445267.1142166667 * t - t2 * 0.001436111132303874;
  ...
  _proto.calcayan = function calcayan(jd) {
    var t = (jd - 2415020) / 36525;
    var om = 259.183275 - 1934.142008333206 * t + 0.0020777778 * t * t + 22222222e-13 * t * t * t;
    var ls = 279.696678 + 36000.76892 * t + 3025e-7 * t * t;
  ...
  _proto.calData = function calData(jd) {
    ...
    if (z2 < 2299161) a = z2;
      ...
      alf = Math.floor((z2 - 186721625e-2) / 36524.25);
  ...
  _proto.lunarPhase = function lunarPhase(jd) {
    var Lmoon = this.moon(jd);
    var Lsun = this.sun(jd);
    return this.fix360(Lmoon - Lsun);
  };
  return MhahPanchangImpl2;
}();
var MhahCalculateFunc = function() {
  function MhahCalculateFunc2() {
  }
  var _proto = MhahCalculateFunc2.prototype;
  ...
  return MhahCalculateFunc2;
}();
var MhahLocalConstant = function MhahLocalConstant2() {
  this.Ritu = {
...
var MhahPkgConstants = function MhahPkgConstants2() {
  this.GanaMatched = [[6, 6, 0], [5, 6, 0], [1, 0, 6]];
};
var MhahSunMoonTimer = function() {
  function MhahSunMoonTimer2() {
    ...
    this.dayMs = 1e3 * 60 * 60 * 24;
    this.J1970 = 2440588;
    this.J2000 = 2451545;
    this.J0 = 9e-4;
    this.e = this.rad * 23.4397;
  }
  var _proto = MhahSunMoonTimer2.prototype;
  ...
  _proto.solarTransitJ = function solarTransitJ(ds, M, L) {
    return this.J2000 + ds + 53e-4 * Math.sin(M) - 69e-4 * Math.sin(2 * L);
  };
  ...
  _proto.eclipticLongitude = function eclipticLongitude(M) {
    var C = this.rad * (1.9148 * Math.sin(M) + 0.02 * Math.sin(2 * M) + 3e-4 * Math.sin(3 * M)), P = this.rad * 102.9372;
  ...
  return MhahSunMoonTimer2;
}();
var MhahCalendar = function() {
  function MhahCalendar2() {
  }
  var _proto = MhahCalendar2.prototype;
  ...
  return MhahCalendar2;
}();
var MhahPanchang = function() {
  function MhahPanchang2() {
  ...
  var _proto = MhahPanchang2.prototype;
  ...
  return MhahPanchang2;
}();
export {
  MhahPanchang
};
//# sourceMappingURL=mhah-panchang.js.map
